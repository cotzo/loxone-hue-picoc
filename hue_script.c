...
#define TRANSITION_TIME (10)          // Fade duration in multiple of 100ms
#define ENABLE_REFRESH_ALL_LIGHTS (0) // Refresh all lights approximately every 60s (to keep hue lights in sync with Loxone Miniserver, if they are changed via the Hue app)
...
void SetOnOff(int id, int state, int isGroup)
{
    ...
    if (0 == state)
    {
        sprintf(command, "{\"on\": false, \"transitiontime\": %d}", TRANSITION_TIME);
    }
    else
    {
...
void SetDim(int id, int brightness, int isGroup)
{
    ...
    else
    {
        ...
        sprintf(command, "{\"on\": true, \"bri\": %d, \"transitiontime\": %d}", TRANSITION_TIME);
        SendCommand(isGroup, id, command);
    }
}
...
void SetTunable(int id, int value, int isGroup)
{
    ...
    {
        sprintf(command, "{\"on\": true, \"bri\": %d, \"ct\": %d, \"transitiontime\": %d}", brightness, temperature, TRANSITION_TIME);
        SendCommand(isGroup, id, command);
    }
}
